...
def check_api_keys() -> dict[str, str]:
    ...
    for key_name in ["EULERPOOL_API_KEY", "FMP_API_KEY", "GEMINI_API_KEY"]:
    ...
    all_present = True
    for key_name, desc in required_keys.items():
        if key_name in keys:
    ...
    if not all_present:
        ...
        # Save to .env
        if any(k in keys for k in required_keys):
            try:
    ...
    if missing:
        _err(f"Missing required keys: {', '.join(missing)}")
...
def main() -> int:
    ...
    required_keys = ["EULERPOOL_API_KEY", "FMP_API_KEY", "GEMINI_API_KEY"]
    if not all(k in keys for k in required_keys):
        _err("Cannot proceed without all API keys.")
        return 1
