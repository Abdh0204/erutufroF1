...
Builds a daily-frequency DataFrame for each entity by:
  ...
  2. Starting with price data (FMP OHLCV for target, Eulerpool quotes
     for linked entities).
...
# Quote / price columns expected from FMP or Eulerpool
QUOTE_FIELDS = (
...
def _align_price_data(
    ...
    """Align price data (FMP OHLCV or Eulerpool quotes) to the daily index.
...
def build_entity_daily_cache(
    ...
    use_fmp_prices:
        If True, use ``entity.fmp_ohlcv`` as the price source (target).
    ...
    # 1. Price data
    price_source = entity.fmp_ohlcv if use_fmp_prices else entity.quotes
    ...
    if not force_rebuild and target_cache_path.exists():
        ...
        try:
            result.target_daily = build_entity_daily_cache(
                extraction.target, daily_index, use_fmp_prices=True,
            )
        ...
        for idx, (isin, entity) in enumerate(extraction.linked.items(), 1):
            ...
            try:
                result.linked_daily[isin] = build_entity_daily_cache(
                    entity, daily_index, use_fmp_prices=False,
                )
            except LookAheadError as exc:
                logger.error("FATAL: %s", exc)
                result.errors.append({
