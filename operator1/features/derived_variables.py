...
def _compute_cash_reality(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Trailing 4-quarter (approx 252 business days) FCF
    # Since statements are as-of, we use a rolling sum over the last ~252 days
    # which approximates TTM from daily-aligned periodic data.
    # A simpler approximation: just use the as-of FCF value directly
    # (since each day already has the latest statement applied).
    df["free_cash_flow_ttm_asof"] = df["free_cash_flow"]
    df["is_missing_free_cash_flow_ttm_asof"] = df["is_missing_free_cash_flow"]
